...
class SentenceChunker:
    ...
    def chunk(self, text: str) -> list[str]:
        ...
        # Keep the sentence-ending punctuation in the sentence.  The final
        # alternative also handles text whose last sentence has no punctuation.
...
class RecursiveChunker:
    ...
    def _split(self, current_text: str, remaining_separators: list[str]) -> list[str]:
        ...
        for candidate in separators:
            if candidate and candidate in current_text:
                separator = candidate
                break

        # No usable separator remains: fall back to fixed-size character cuts.
        if separator is None:
            step = max(1, self.chunk_size)
            return [current_text[i : i + self.chunk_size] for i in range(0, len(current_text), step)]
        ...
        for piece in pieces:
            ...
            if not piece:
                continue
            candidate = f"{pending}{separator}{piece}" if pending else piece
            if len(candidate) <= self.chunk_size:
                pending = candidate
                continue

            if pending:
                result.extend(self._split(pending, next_separators))
            pending = piece
